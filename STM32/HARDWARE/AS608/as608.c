		...
		if(ensure == 0x00) //获取图像成功
		{
			...
			if(ensure == 0x00) //生成特征成功
			{
				...
				if(ensure == 0x00) //搜索成功
				{
					...
					OLED_ShowCH(8, 4, (u8*)str);
					if(seach.pageID == 0)
					{
						uart_sendbyte('4');
					}
					else if(seach.pageID == 1)
					{
						uart_sendbyte('5');
					}
					delay_ms(4000);
				...
				else
				{
					alarm_num += 1;
					OLED_Clear();
					OLED_ShowCH(1, 3, "Validation Fail");
					uart_sendbyte('2');	//'2'代表指纹验证失败一次
					delay_ms(1000);
					if(alarm_num == 3)
					{
						alarm_num = 0;
						OLED_Clear();
						OLED_ShowCH(10, 3, "Check Finger");
						uart_sendbyte('3');	//'3'代表指纹验证失败三次
						delay_ms(1500);
						break;
					}
					delay_ms(1500);
